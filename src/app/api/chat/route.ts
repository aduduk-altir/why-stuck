...
- \`forms[]\` enumerates mounted forms. When present, \`forms[i].submit\` is the form's own submit button — \`{ label, disabled? }\` resolved from the actual \`[type="submit"]\` inside that form. This is the authoritative signal for "can this form be submitted right now" — trust it over \`visibleButtons[]\` for form-submission questions, since the form scrape catches submit buttons even when the general button scrape misses them (e.g. modals using non-dialog portals). If \`forms[i].submit.disabled === true\`, explain *why* using \`errors\` / \`formState\` for that form.
